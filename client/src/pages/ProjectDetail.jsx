...
export default function ProjectDetail({data}){
  ...
  return <main className={`detail-page detail-${p.accent||'mint'}`}>
    <section className="detail-hero">
      <Link className="back-link" to="/"><ArrowLeft size={16}/> Back to Projects</Link>
      <div className="detail-hero-grid">
        <div className="detail-hero-copy">
          <p className="eyebrow">CASE STUDY / {p.no||'01'}</p>
          <p className="detail-status">● {p.status||'IN PROGRESS'} &nbsp;·&nbsp; {p.year||''}</p>
          <h1>{p.title}</h1>
        ...
        <div className="detail-hero-visual">
          <div className="detail-window">
            ...
            {current.image?<img src={current.image} alt={current.title||`${p.title} preview`}/>:<div className="empty-visual"><Sparkles size={38}/><span>Add screenshots from Admin</span></div>}
          </div>
        </div>
      </div>
    </section>
    ...
    <section className="case-layout">
      <div className="case-gallery-main">
        <div className="case-image" onClick={()=>current.image&&setLightbox(true)}>
          ...
          {current.image&&<span className="zoom-hint">CLICK TO EXPAND ↗</span>}
        </div>
        {images.length>1&&<div className="thumb-row">{images.map((s,i)=><button type="button" key={`${s.image}-${i}`} className={i===index?'selected':''} onClick={()=>setIndex(i)}><img src={s.image} alt={s.title||''}/><span>{String(i+1).padStart(2,'0')}</span></button>)}</div>}
      </div>

      <aside className="case-side">
        <p className="eyebrow">PROJECT INTELLIGENCE</p>
        <div className="fact"><span>STATUS</span><b>{p.status||'—'}</b></div>
        <div className="fact"><span>YEAR</span><b>{p.year||'—'}</b></div>
        <div className="fact"><span>TYPE</span><b>{p.type||'—'}</b></div>
    ...
    <section className="case-story">
      ...
      <div>
        <p className="story-copy">{p.description||p.summary}</p>
        <div className="feature-grid">{(p.features||[]).map((f,i)=><article key={`${f}-${i}`}><span>{String(i+1).padStart(2,'0')}</span><b>{f}</b><small>Designed as part of the product workflow and managed from the portfolio cockpit.</small></article>)}</div>
      </div>
    </section>
    ...
    {images.length>0&&<section className="case-gallery">
      ...
      <div className="case-sections">
        {images.map((s,i)=><article className="case-section" key={`${s.image}-${i}`}>
          <div className="case-section-number">{String(i+1).padStart(2,'0')}</div>
